    ...
    <div className="grid gap-4">
      {tiers.map((tier) => (
        <div 
          ...
          className={`p-4 border-2 rounded-lg cursor-pointer hover:border-primary relative
            ${selectedTier.period === tier.period ? 'border-primary bg-primary/5' : 'border-border'}`}
        >
...
export function RechargeCalculator({ exchangeRate, serviceFee ,onCalculate }: RechargeCalculatorProps) {
  ...
  }, [amount, exchangeRate, serviceFee]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2 text-sm text-gray-600">
        ...
        <p>基础金额：¥{calculation.baseAmount.toFixed(2)}</p>
        <p>服务费({serviceFee}%)：¥{calculation.feeAmount.toFixed(2)}</p>
        <p className="text-lg font-bold text-black">总计：¥{calculation.total.toFixed(2)}</p>
      </div>
    </div>
  );
}